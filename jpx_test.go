...
func TestAFormatNothingHereDecodesIsNotDrawn(t *testing.T) {
	// JPEG 2000 used to be the example of this. What is left is JBIG2, which
	// 12 of 250 court filings and 1 of 222 scanned books carry — a real gap,
	// and one that draws nothing rather than drawing noise.
	wr := reader.NewWriter("1.7")
	pagesRef := wr.Reserve()
	img := wr.Add(&reader.Stream{Dict: reader.Dict{
		"Type": reader.Name("XObject"), "Subtype": reader.Name("Image"),
	...
	if ink := inked(pic); ink != 0 {
		t.Errorf("%d pixels drawn from a format nothing here decodes", ink)
	}
}
